...
import { allowedURLPathsAuthToken } from '../../config/allowedURLPathConfig';
import { ApiResponse } from '../../config/responseConfig';
import { organizationDBService } from '../../db/postgres/organizationDB';
export const jwtAuthMiddleware = async (req: Request, res: Response, next: NextFunction): Promise<void | ApiResponse> => {
  ...
  return passport.authenticate('jwt', { session: false, failWithError: true }, async (err, user, info) => {
    ...
    if (!isValidOrganization)
      return ApiResponse.unAuthorized(res, 'ORGANIZATION_ASSOCIATION_NOT_FOUND_ERROR. Please contact your organization admin');
    req.user = user;
    return next();
  })(req, res, next);
};
